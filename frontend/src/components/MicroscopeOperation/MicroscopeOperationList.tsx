...
interface MicroscopeOperationListProps {
  workflows: MicroscopeOperationWorkflow[];
  loading: boolean;
  error: string | null;
  onSelectWorkflow: (workflow: MicroscopeOperationWorkflow) => void;
  onDeleteWorkflow: (id: string) => void;
  selectedWorkflow: MicroscopeOperationWorkflow | null;
}

export function MicroscopeOperationList({ 
  workflows, 
  loading, 
  error, 
  onSelectWorkflow, 
  ...
  if (workflows.length === 0) {
    return (
      <div className="text-center py-8 text-gray-500">
        <p>暂无显微镜操作工作流</p>
        <p className="text-sm mt-1">使用 MCP 工具创建一个工作流开始吧！</p>
      </div>
    );
  }
        ...
        >
          ...
          >
            ...
            <div className="text-xs text-gray-500">
              创建时间: {formatTimestamp(workflow.created_at)}
            </div>
          </div>
        </div>
      ))}
