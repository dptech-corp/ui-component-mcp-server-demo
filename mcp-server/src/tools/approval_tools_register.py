"""
Registration function for approval tools in the MCP server.
"""
...
def register_approval_tools(mcp, redis_client: RedisClient):
    ...
    @mcp.tool()
    async def wait_for_approval(description: str) -> dict:
        """Request human approval before proceeding with an action"""
        return await wait_for_approval(description)
    
    @mcp.tool()
    async def check_approval_status(ticket_id: str) -> dict:
        """Check the status of a pending approval request"""
        return await check_approval_status(ticket_id)
    
    print("Registered approval tools: wait_for_approval, check_approval_status")
